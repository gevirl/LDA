...
public class ChibEstimator implements Callable {
    ...
    public double estimateLogProb() {
        ...
        double logPzStar = Gamma.logGamma(alpha) - Gamma.logGamma(Nd + alpha);
//        double logPzStar = Gamma.logGamma(V * alpha) - Gamma.logGamma(Nd + V * alpha);
        for (int t = 0; t < T; ++t) {
            logPzStar = logPzStar + Gamma.logGamma((double) NzStar[t] + alpha/T);
        }
        logPzStar = logPzStar - T * Gamma.logGamma(alpha/T);
    ...
    public double probZ(int topic, int vocab, int[] N) {
        double p = phi[topic][vocab] * (N[topic] + alpha/T);
        if (p < 0.0) {
            int iuashdfui = 0;
        }
        return p;
